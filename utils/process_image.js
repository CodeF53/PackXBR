...
/**
 * Calls `processImage` determining Tile/Relayer/Skip based on image path/name
 */
export async function processImageAutoArgs({ pngFile, scaleFactor = 4 }) {
  const { name } = pngFile
  ...
  // process the image
  return await processImage({ pngFile, scaleFactor, tile, relayer, skip })
}
 ...
 * @param {object} tile - An object containing information how each edge of the image should be treated.
 *   Each edge can be `void`, `wrap`, `extend`, or `mirror`
 *   example: `{ n: 'void', s: 'void', e: 'void', w: 'void' }`
 * @param {boolean} relayer - If a nearest neighbor upscale should be rendered below the result to smooth corners
 * @param {boolean} semiTranslucencyCulling - If semiTranslucent pixels should be culled
 * @param {boolean} skip - If semiTranslucent pixels should be culled
 * @returns {canvas} A canvas containing the processed image
 */
export async function processImage(
  { pngFile, scaleFactor, tile, relayer, semiTranslucencyCulling, skip },
) {
  // Load the png file into an Image object
  const img = new Image()
  img.src = URL.createObjectURL(new Blob(pngFile.data, { type: 'image/png' }))
  ...
  try {
    ...
    if (semiTranslucencyCulling)
      shouldCull = !containsSemiTranslucency(canvas)
    ...
    // TODO: switch return to { name: name, data: upscaledData as ArrayBuffer }
    return canvas
  }
  catch (error) {
